...
public class PatternStore {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(ps.getNumPatternsRepresentableBy(110));
	}
	...
	public List<Pattern> getPatternsAuthorSorted() {
		...
		Comparator<Pattern> compAuthName = Comparator.comparing(Pattern::getAuthor).thenComparing(Pattern::getName);
		Collections.sort(mPatterns, compAuthName);
		return new LinkedList<Pattern>(mPatterns);
	}
	...
	public List<String> getNumPatternsRepresentableBy(int x) {
		Collections.sort(mPatterns);
		List<Pattern> patterns = new LinkedList<Pattern>(mPatterns);
		List<String> list = patterns.stream().filter(y -> y.getWidth() * y.getHeight() <= x)
				.map(y -> y.getName() + " " + y.getHeight() + " " + y.getWidth()).collect(Collectors.toList());
		return list;
	}
